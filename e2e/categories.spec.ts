...
describe('Manage categories page', () => {
  test('there is a link to the manage categories page in the add category modal', async ({
    page,
  }) => {
  ...
  test('there is a link to the manage categories page in the edit category modal', async ({
    page,
  }) => {
    await page.goto('http://localhost:3030/?demo&faker');
    ...
    await page.getByTestId('edit-expense-btn').first().click();

    await page
      .getByTestId('edit-expense-dialog')
      .getByRole('link', { name: 'Gestisci categorie' })
      .click();
      ...
      categories.map(async (category) => {
        ...
        await Promise.all(
          subcategories[category].map((subcategory) =>
            expect(page.getByText(subcategory).first()).toBeVisible(),
          ),
        );
      }),
    );
...
describe('subcategories', () => {
  test('a subcategory can be deleted', async ({ page }) => {
    ...
    await page
      .getByRole('button', { name: 'cancella sottocategoria xpug' })
      .click();
  ...
  test('a subcategory can be modified', async ({ page }) => {
    ...
    await page.getByText('xpug').click();
    await page
      .getByTestId('subcategory-xpug')
      .getByTestId('subcategory-edit-input')
      .fill('xpug2');
    await page
      .getByTestId('subcategory-xpug')
      .getByRole('button', { name: 'salva sottocategoria xpug' })
      .click();
